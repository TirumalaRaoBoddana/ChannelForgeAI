...
export function makeImageRoute(opts: ImageRouteOpts) {
  return async function POST(req: NextRequest) {
    ...
    // Staged projects carry a Brand Identity Object instead of a full analysis.
    if (!project.analysis && !project.brandIdentity) {
      return fail("NO_BRAND", "Choose a channel name and brand colors before generating assets.", 400);
    }
