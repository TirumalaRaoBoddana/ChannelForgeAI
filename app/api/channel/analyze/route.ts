...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const project: Project = {
      ...
      stage: "complete",
      nameGenerations: [], selectedNameId: null, palettes: [], brandIdentity: null,
      chosenAssets: [], tagline: null, description: null, keywords: null, thumbnailGuide: null,
    ...
    // guestToken lets cookie-blocked clients (preview iframe) navigate to the
    // kit page with the identity that owns the project (?g= param).
    return ok({ projectId: project.id, analysis: result.data, provider: result.provider, quota, guestToken: idn.guestToken });
  } catch (e) {
