...
export function KitView(props: KitViewProps) {
  ...
  const regenerate = async (assetType: "logo" | "banner" | "watermark") => {
    ...
    const res = await fetch(`/api/generate/${assetType}?${guestQuery()}`, {
      method: "POST", headers: { "Content-Type": "application/json", "x-cf-client": "1", "x-cf-guest": clientGuestToken() },
  ...
  const AssetActions = ({ assetType, assetId, filename }: { assetType: "logo" | "banner" | "watermark"; assetId?: string; filename: string }) => (
    <div className="mt-4 flex flex-wrap gap-2">
      {/* Regenerate mutates the project — hidden on the public demo, which no visitor owns. */}
      {!isDemo && (
      ...
      {assetId && (
        <a href={`/api/assets/${assetId}/file?${guestQuery()}`} download={filename} className="btn-secondary !px-3 !py-2 !text-xs" onClick={() => void fetch("/api/track", { method: "POST", headers: { "Content-Type": "application/json", "x-cf-client": "1" }, body: JSON.stringify({ name: "download_clicked", props: { kind: assetType } }) }).catch(() => {})}>
    ...
    <div className="section py-10">
      ...
      <header className="mb-8">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <a href={`/api/projects/${props.project.id}/download?${guestQuery()}`} className="btn-primary" onClick={() => void fetch("/api/track", { method: "POST", headers: { "Content-Type": "application/json", "x-cf-client": "1" }, body: JSON.stringify({ name: "download_clicked", props: { kind: "full_kit" } }) }).catch(() => {})}>
